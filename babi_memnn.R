...
## random test. this is not really testing since its using the validation set.
nrow <- nrow(test_vec$questions)
idx <- sample(1:nrow, 1) 
story <- matrix(test_vec$stories[idx,],nrow=1,ncol=68)
question <- matrix(test_vec$questions[idx,], nrow=1, ncol=4)
ans <- test_vec$answers[idx,]

pred <- model %>% predict(x=list(story, question))

id <- which(pred %in% max(pred))
