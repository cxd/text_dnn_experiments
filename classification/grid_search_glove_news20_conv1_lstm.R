...
base_model_name <- "news_glove_conv1d_lstm"
...
for (i in 1:length(embedding)) {
  ...
  for (j in 1:num_samples) {
    ...
    glove <-if (edim == 50) read_glove_model(model50)
            else if (edim == 100) read_glove_model(model100)
            else read_glove_model(model300)
